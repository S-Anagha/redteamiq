...
import { TOOLS, DEMO_TARGET } from '../data/tools.js'
...
export default function InputScreen({ onRun, mode, setMode, endpoint, setEndpoint }) {
  ...
  const loadDemo = () => {
    setSystemPrompt(DEMO_TARGET.system_prompt)
    setTools(DEMO_TARGET.tools)
  }

  const canRun = systemPrompt.trim().length > 0 && tools.length > 0 &&
    (mode === 'mock' || endpoint.trim().length > 0)
    ...
    <div className="mx-auto max-w-3xl px-6 py-10">
      ...
      <Panel className="p-6">
        <div className="mb-2 flex items-center justify-between">
          ...
          <button
            onClick={loadDemo}
            className="rounded-md border border-[#1f2937] bg-[#0f1521] px-2.5 py-1 text-xs text-cyan-300 transition hover:border-cyan-500/50 hover:text-cyan-200"
          >
            ⚡ Load vulnerable demo target
          </button>
        </div>
        <textarea
          value={systemPrompt}
        ...
        <div className="mt-6 rounded-lg border border-[#1f2937] bg-[#0a0e14] p-3.5">
          ...
          {mode === 'live' && (
            <input
              ...
              placeholder="https://your-backend/analyze"
              className="mt-3 w-full rounded-md border border-[#1f2937] bg-[#0a0e14] px-3 py-2 font-mono text-xs text-gray-200 outline-none focus:border-cyan-500/50"
            />
          )}
        </div>
